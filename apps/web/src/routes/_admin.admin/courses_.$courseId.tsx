  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <h2 className="font-display text-lg uppercase tracking-widest text-muted-foreground">Modules & Lessons</h2>
      ...
      {isCreating && (
        <Panel accent="cyan" className="mb-6">
          ...
          <form onSubmit={handleCreateLesson} className="mt-4 space-y-4 max-w-xl">
            <div>
              <label className="block text-xs uppercase tracking-widest text-muted-foreground mb-1">Lesson Title</label>
            ...
            <div>
              ...
              <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full rounded-md border border-border bg-background/50 px-3 py-2 text-sm text-foreground focus:border-neon-cyan focus:outline-none" rows={3} />
            </div>
            <div>
              <label className="block text-xs uppercase tracking-widest text-muted-foreground mb-1">Video URL (YouTube/Vimeo/Direct)</label>
              <input required type="url" value={formData.videoUrl} onChange={e => setFormData({...formData, videoUrl: e.target.value})} placeholder="https://..." className="w-full rounded-md border border-border bg-background/50 px-3 py-2 text-sm text-foreground focus:border-neon-cyan focus:outline-none" />
            </div>
            <div className="flex gap-2 justify-end pt-2">
              <Button type="button" variant="ghost" onClick={() => setIsCreating(false)}>Cancel</Button>
              <Button type="submit" variant="neon">Save Lesson</Button>
            </div>
          </form>
        </Panel>
      )}
      ...
      <div className="space-y-3">
        {course.lessons.map((lesson, index) => (
          <Panel key={lesson.id} className="p-4 flex items-center justify-between hover:border-neon-cyan/50 transition-colors">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <p className="truncate font-display text-base font-bold text-foreground">{lesson.title}</p>
            ...
            <div className="flex shrink-0 ml-4">
              <Button variant="ghost" size="icon" onClick={() => handleDeleteLesson(lesson.id)} className="text-red-500 hover:text-red-400 hover:bg-red-500/10">
